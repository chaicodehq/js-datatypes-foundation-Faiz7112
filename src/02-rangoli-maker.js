...
export function extractRangoliCenter(design, start, end) {
  if (typeof design !== "string" || typeof start !== "number" || typeof end !== "number" || start < 0 || end < 0 || !Number.isInteger(start) || !Number.isInteger(end)) return "";
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  let output2 = colorString.split(oldSep);
  return output2.join(newSep);
}
...
export function makeRangoliBorder(char, length) {
  ...
  let newChar = char.repeat(Math.ceil(length / char.length));
