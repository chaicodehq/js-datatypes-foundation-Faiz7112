...
export function updatePrices(menu, increase) {
  ...
  const propertiesArray = Object.entries(menu);
  const updatedArray = propertiesArray.map(([key, value]) => [key, value + increase]);
  const newMenu = Object.fromEntries(updatedArray);
  return newMenu;
}
